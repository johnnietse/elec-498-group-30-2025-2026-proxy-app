                                          ...
                                          int fsync_each_chunk)
{
  ...
  //
  // Strategy: write real checkpoint data repeatedly until target_duration_sec
  // is reached. No sleep, no padding — duration is determined purely by how
  // much data needs to be written and how fast the storage can absorb it.
  // This means CPU frequency changes should NOT affect IO duration (storage
  // is the bottleneck), but WILL affect power draw — exactly what we want
  // to measure for DVFS energy savings during IO phases.
  //
  // To control IO duration, set --ckpt_chunk_mb large enough that writing
  // fills the target time at your storage's natural throughput (~300 MB/s
  // on lscratch means 9 GB fills ~30s). The sleep_us parameter is ignored.

  if(chunk_bytes == 0) chunk_bytes = 1024 * 1024; // Default 1 MB chunks
  // target_duration_sec is now used only as a minimum — we keep writing
  // real data passes until we have both written at least one full pass
  // AND elapsed >= target_duration_sec. If target <= 0, write exactly once.
  ...
  if(me == 0) {
    ...
    std::printf("Data per rank: %.2f MB\n", all_data.size() / (1024.0 * 1024.0));
    std::printf("Target duration: %.1f seconds (storage-bound, no sleep)\n", target_duration_sec);
    std::printf("NOTE: sleep_us parameter ignored — IO is now purely storage-bound\n");
  }

  // Keep writing passes of real data until target duration is met.
  // Each pass truncates and rewrites the file to avoid unbounded disk growth.
  do {
    // On passes after the first, seek back to start and truncate
    if(pass > 0) {
      if(::lseek(fd, 0, SEEK_SET) < 0 ||
         ::ftruncate(fd, 0) < 0) {
        // If seek/truncate fails just keep appending — not ideal but safe
        if(me == 0) {
          std::fprintf(stderr, "WARNING: Could not rewind file for pass %d, appending instead\n", pass);
        }
      }
    }
    pass++;
    ...
    while(pass_bytes < data_size) {
      size_t bytes_remaining = data_size - pass_bytes;
      size_t this_chunk_size = (bytes_remaining < chunk_bytes) ? bytes_remaining : chunk_bytes;

      const unsigned char* write_ptr = all_data.data() + pass_bytes;
    ...
    if(me == 0 && target_duration_sec > 0.0) {
      std::printf("  Pass %d complete: %.2f MB written this pass, elapsed=%.2f/%.1fs\n",
                  pass,
                  static_cast<double>(pass_bytes) / (1024.0 * 1024.0),
                  elapsed,
                  target_duration_sec);
    }

  } while(target_duration_sec > 0.0 && elapsed < target_duration_sec);
  
  // Unused parameter — kept in signature for API compatibility
  (void)sleep_us;
  ...
  if(me == 0) {
    ...
    std::printf("Checkpoint data per rank: %.2f MB\n", data_mb);
    std::printf("Total written per rank: %.2f MB (%d passes)\n", total_mb, pass);
    std::printf("Number of chunks: %zu\n", chunk_count);
    std::printf("Average bandwidth: %.2f MB/s per rank\n", avg_bw);
    std::printf("(IO duration was storage-bound — no artificial sleep used)\n");
    std::printf("=====================================\n\n");
  }
    ...
    for(n = 0; n < ntimes; n++) {
      ...
      if(ckpt_at_end && (n + 1) == checkpoint_step) {
        #pragma omp master
        {
          ...
          if(me == 0) {
            std::printf("\n========================================\n");
            std::printf("Reached mid-simulation checkpoint at timestep %d (out of %d)\n", n+1, ntimes);
