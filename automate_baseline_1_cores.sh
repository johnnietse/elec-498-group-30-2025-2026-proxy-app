...
if [ ! -f $CSV_FILE ]; then
    echo "Run_Number,Execution_Time_Sec_1_Core,Total_Energy_J_1_Core,Avg_Power_W_1_Core" > $CSV_FILE
fi
...
do
    ...
    # Run simulation
<<<<<<< HEAD
    mpirun -np 1 --report-bindings ./miniMD_openmpi -i in.lj.miniMD >> $LOG_FILE 2>&1
=======
    $EXEC_WRAPPER mpirun -np 1 --map-by core --bind-to core --report-bindings ./miniMD_openmpi -i in.lj.miniMD --ckpt 200 >> $LOG_FILE 2>&1
