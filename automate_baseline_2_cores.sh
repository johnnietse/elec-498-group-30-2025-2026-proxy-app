...
# Configuration
CSV_FILE="2_cores_simulation_results_${SLURM_JOB_ID:-manual}.csv"
...
if [ ! -f $CSV_FILE ]; then
    echo "Run_Number,Execution_Time_Sec_2_Cores,Total_Energy_J_2_Cores,Avg_Power_W_2_Cores" > $CSV_FILE
fi
...
do
    ...
    # Run simulation
<<<<<<< HEAD
    mpirun -np 2 --report-bindings ./miniMD_openmpi -i in.lj.miniMD >> $LOG_FILE 2>&1
=======
    $EXEC_WRAPPERmpirun -np 2 --map-by core --bind-to core --report-bindings ./miniMD_openmpi -i in.lj.miniMD --ckpt 200 >> $LOG_FILE 2>&1
