...
int main(int argc, char** argv)
{
  ...
  for(int i = 0; i < argc; i++) {
    ...
    if((strcmp(argv[i], "--ckpt_sleep_ms") == 0)) {
      ...
      if(me == 0) {
        std::printf("WARNING: --ckpt_sleep_ms is ignored. IO duration is now storage-bound.\n"
                    "         Use --ckpt_io_duration to control how long IO runs.\n");
      }
      continue;
    }
    ...
    if((strcmp(argv[i], "-h") == 0) || (strcmp(argv[i], "--help") == 0)) {
      ...
      printf("\t--ckpt_dir <DIR>:             checkpoint directory (default: chk)\n");
      printf("\t--ckpt_io_duration <SEC>:     target I/O duration in seconds (default: 30)\n");
      printf("\t                              IO is storage-bound: real data written repeatedly\n");
      printf("\t                              until this duration is reached. Set to 0 for\n");
      printf("\t                              a single pass (write data once, no repeat).\n");
      printf("\t--ckpt_chunk_mb <MB>:         chunk size per write call in MB (default: 1)\n");
      printf("\t--ckpt_sleep_ms <MS>:         IGNORED - IO is now purely storage-bound\n");
      printf("\t--ckpt_fsync <0|1>:           fsync after each chunk (default: 0)\n");
      printf("\n  Examples:\n");
      printf("\t# 30 seconds of storage-bound IO, 4MB chunks:\n");
      printf("\t  mpirun -np 32 ./miniMD --ckpt_io_duration 30 --ckpt_chunk_mb 4\n\n");
      printf("\t# Single pass - write checkpoint data once as fast as storage allows:\n");
      printf("\t  mpirun -np 32 ./miniMD --ckpt_io_duration 0\n\n");
      printf("\t# With fsync to ensure data hits disk each chunk:\n");
      printf("\t  mpirun -np 32 ./miniMD --ckpt_io_duration 30 --ckpt_chunk_mb 4 --ckpt_fsync 1\n\n");
  ...
  // Pass sustained I/O parameters to Integrate
