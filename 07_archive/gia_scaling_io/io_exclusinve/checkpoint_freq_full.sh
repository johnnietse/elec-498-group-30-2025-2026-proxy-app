...
FREQ_1200="1200000"  # 1.2 GHz
FREQ_1600="1600000"  # 1.6 GHz
FREQ_2000="2000000"  # 2.0 GHz
...
if [ ! -w /sys/devices/system/cpu/cpu0/cpufreq/scaling_governor ]; then
    echo ""
    echo "WARNING: Cannot write to CPU frequency files!"
    echo "You may need sudo/root privileges to change CPU frequencies"
    echo "Proceeding anyway - frequencies may not actually change"
fi
...
set_all_cores_frequency() {
    ...
    echo "Setting all cores to ${freq_name}..."
    ...
    for (( core=0; core<TOTAL_CORES; core++ )); do
        ...
        if [ -w "$gov_file" ]; then
            ...
            if [ $? -eq 0 ]; then
                echo "$target_freq" > "$speed_file" 2>/dev/null
                if [ $? -eq 0 ]; then
                    ((set_count++))
                else
                    ((fail_count++))
...
reset_frequencies() {
    echo "Resetting CPU frequencies to default..."
...
run_at_frequency() {
    ...
    # Set frequency
    set_all_cores_frequency "$freq_khz" "$freq_name"
    sleep 2
    ...
    touch ".running_${freq_name}_${run}"
    sample_frequency "${freq_name}_${run}" &
    FREQ_SAMPLER_PID=$!
    ...
    NUM_FILES=$(find "$CKPT_DIR" -type f -name "*.bin" | wc -l)
    ...
    if [ -f ".freq_samples_${freq_name}_${run}" ]; then
        AVG_FREQ=$(awk '{sum+=$2; count++} END {if(count>0) print int(sum/count); else print 0}' ".freq_samples_${freq_name}_${run}")
        MIN_FREQ=$(awk 'BEGIN{min=999999} {if($2<min && $2>0) min=$2} END {print int(min)}' ".freq_samples_${freq_name}_${run}")
        MAX_FREQ=$(awk 'BEGIN{max=0} {if($2>max) max=$2} END {print int(max)}' ".freq_samples_${freq_name}_${run}")
        ...
        mv ".freq_samples_${freq_name}_${run}" "freq_samples_${freq_name}_${CORES}cores_run${run}_${SLURM_JOB_ID}.txt"
    else
        echo "  CPU Frequency: Sampling data not available"
    fi
...
for run in $(seq 1 $NUM_RUNS); do
    ...
    CSV_LINE=$(run_at_frequency "$FREQ_1200" "1200MHz" "$run")
    if [ $? -eq 0 ]; then
    ...
    CSV_LINE=$(run_at_frequency "$FREQ_1600" "1600MHz" "$run")
    if [ $? -eq 0 ]; then
    ...
    CSV_LINE=$(run_at_frequency "$FREQ_2000" "2000MHz" "$run")
    if [ $? -eq 0 ]; then
...
# Checkpoint bins directories
for freq in "1200MHz" "1600MHz" "2000MHz"; do
    for dir in checkpoint_bins_${freq}_run*_${SLURM_JOB_ID}; do
        [ -d "$dir" ] && FILES_TO_TAR="$FILES_TO_TAR $dir"
    done
done
