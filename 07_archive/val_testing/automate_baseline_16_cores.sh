...
if [ ! -f $CSV_FILE ]; then
    echo "Run_Number,Execution_Time_Sec_16_Cores,Total_Energy_J_16_Cores,Avg_Power_W_16_Cores" > $CSV_FILE
fi
...
do
    ...
    # Run simulation
    $EXEC_WRAPPER mpirun -np 16 --map-by core --bind-to core --report-bindings ./miniMD_openmpi -i in.lj.miniMD >> $LOG_FILE 2>&1
