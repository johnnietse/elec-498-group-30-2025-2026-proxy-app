...
void output(In &, Atom &, Force *, Neighbor &, Comm &, Thermo &, Integrate &,
            Timer &, int);
...
int main(int argc, char **argv) {
  ...
  int ckpt_interval = 0;        // DEPRECATED - kept for compatibility
  const char *ckpt_dir = "chk"; // default output directory

  // Sustained I/O parameters (Gia's I/O phase)
  double ckpt_io_duration_sec = 30.0; // Default 30 seconds
  int ckpt_chunk_mb = 1;              // Default 1 MB chunks
  int ckpt_sleep_ms = 100;            // Default 100 ms sleep
  int ckpt_fsync_chunks = 0;          // Default: don't fsync each chunk

  // Communication phase parameters (Johnnie)
  int comm_phase_enabled = 1; // Enabled by default
  int comm_standin_mb = 309;  // 309 MB stand-in
  int comm_chunk_kb = 1024;   // 1 MB chunks for MPI send/recv
  ...
  for (int i = 0; i < argc; i++) {
    ...
    if ((strcmp(argv[i], "--ckpt_dir") == 0)) {
      ckpt_dir = argv[++i];
      continue;
    }

    // Sustained I/O CLI flags (Gia)
    if ((strcmp(argv[i], "--ckpt_io_duration") == 0)) {
      ckpt_io_duration_sec = atof(argv[++i]);
      continue;
    }
    if ((strcmp(argv[i], "--ckpt_chunk_mb") == 0)) {
      ckpt_chunk_mb = atoi(argv[++i]);
      continue;
    }
    if ((strcmp(argv[i], "--ckpt_sleep_ms") == 0)) {
      ckpt_sleep_ms = atoi(argv[++i]);
      continue;
    }
    if ((strcmp(argv[i], "--ckpt_fsync") == 0)) {
      ckpt_fsync_chunks = atoi(argv[++i]);
      continue;
    }

    // Communication phase CLI flags (Johnnie)
    if ((strcmp(argv[i], "--comm_phase") == 0)) {
      comm_phase_enabled = atoi(argv[++i]);
      continue;
    }
    if ((strcmp(argv[i], "--comm_standin_mb") == 0)) {
      comm_standin_mb = atoi(argv[++i]);
      continue;
    }
    if ((strcmp(argv[i], "--comm_chunk_kb") == 0)) {
      comm_chunk_kb = atoi(argv[++i]);
      continue;
    }
    ...
    if ((strcmp(argv[i], "-h") == 0) || (strcmp(argv[i], "--help") == 0)) {
      ...
      printf("miniMD is a simple, parallel molecular dynamics (MD) code,\n"
             ...
             "The original authors of miniMD are Steve Plimpton "
             "(sjplimp@sandia.gov) ,\n"
             "Paul Crozier (pscrozi@sandia.gov) with current\n"
             "versions written by Christian Trott (crtrott@sandia.gov).\n\n");
      printf("Commandline Options:\n");
      printf("\n  Execution configuration:\n");
      printf("\t--teams <nteams>:             set number of thread-teams used "
             "per MPI rank (default 1)\n");
      printf("\t-t / --num_threads <threads>: set number of threads per "
             "thread-team (default 1)\n");
      printf(
          "\t--half_neigh <int>:           use half neighborlists (default 1)\n"
          "\t                                0: full neighborlist\n"
          "\t                                1: half neighborlist\n"
          "\t                               -1: original miniMD half "
          "neighborlist force (not OpenMP safe)\n");
      printf("\t-d / --device <int>:          choose device to use (only "
             "applicable for GPU execution)\n");
      printf("\t-dm / --device_map:           map devices to MPI ranks\n");
      printf("\t-ng / --num_gpus <int>:       give number of GPUs per Node "
             "(used in conjuction with -dm\n"
             "\t                              to determine device id: "
             "'id=mpi_rank%%ng' (default 2)\n");
      printf("\t--skip_gpu <int>:             skip the specified gpu when "
             "assigning devices to MPI ranks\n"
             "\t                              used in conjunction with -dm "
             "(but must come first in arg list)\n");
      printf("\t-sse <sse_version>:           use explicit sse intrinsics (use "
             "miniMD-SSE variant)\n");
      printf("\t-gn / --ghost_newton <int>:   set usage of newtons third law "
             "for ghost atoms\n"
             "\t                                (only applicable with half "
             "neighborlists)\n");
      printf("\n  Simulation setup:\n");
      printf("\t-i / --input_file <string>:   set input file to be used "
      ...
      printf("\t-s / --size <int>:            set linear dimension of "
             "systembox\n");
      printf("\t-nx/-ny/-nz <int>:            set linear dimension of "
             "systembox in x/y/z direction\n");
      printf("\t-b / --neigh_bins <int>:      set linear dimension of neighbor "
             "bin grid\n");
      printf("\t-u / --units <string>:        set units (lj or metal), see "
             "LAMMPS documentation\n");
      printf("\t-p / --force <string>:        set interaction model (lj or "
             "eam)\n");
      printf("\t-f / --data_file <string>:    read configuration from LAMMPS "
             "data file\n");

      printf("\n  Miscelaneous:\n");
      printf("\t--check_exchange:             check whether atoms moved "
             "further than subdomain width\n");
      printf("\t--safe_exchange:              perform exchange communication "
             "with all MPI processes\n"
             "\t                                within rcut_neighbor (outer "
             "force cutoff)\n");
      ...
      printf("\n  Sustained I/O Checkpointing (for monitoring):\n");
      printf("\t--ckpt_dir <DIR>:             checkpoint directory (default: "
             "chk)\n");
      printf("\t--ckpt_io_duration <SEC>:     I/O duration in seconds "
             "(default: 30)\n");
      printf("\t--ckpt_chunk_mb <MB>:         chunk size in MB (default: 1)\n");
      printf("\t--ckpt_sleep_ms <MS>:         sleep between chunks in ms "
             "(default: 100)\n");
      printf("\t--ckpt_fsync <0|1>:           fsync after each chunk (default: "
             "0)\n");

      printf("\n  Communication Phase (Johnnie):\n");
      printf("\t--comm_phase <0|1>:           enable/disable comm phase "
             "(default: 1)\n");
      printf("\t--comm_standin_mb <MB>:       stand-in total data in MB "
             "(default: 309)\n");
      printf("\t--comm_chunk_kb <KB>:         MPI send/recv chunk size in KB "
             "(default: 1024)\n");

      printf("\n  Examples:\n");
      printf("\t# 30 seconds of I/O with 1MB chunks, 100ms sleep:\n");
      printf("\t  mpirun -np 4 ./miniMD --ckpt_io_duration 30\n\n");
      printf("\t# 60 seconds of I/O with 2MB chunks, 50ms sleep:\n");
      printf("\t  mpirun -np 4 ./miniMD --ckpt_io_duration 60 --ckpt_chunk_mb "
             "2 --ckpt_sleep_ms 50\n\n");
      printf("\t# Fast I/O (10 seconds, 5MB chunks, no sleep):\n");
      printf("\t  mpirun -np 4 ./miniMD --ckpt_io_duration 10 --ckpt_chunk_mb "
             "5 --ckpt_sleep_ms 0\n\n");
      printf("\t# Communication phase with 500MB stand-in:\n");
      printf("\t  mpirun -np 32 ./miniMD --comm_standin_mb 500\n\n");
      printf("\t# Disable communication phase:\n");
      printf("\t  mpirun -np 32 ./miniMD --comm_phase 0\n\n");
  ...
  if (in.forcetype == FORCEEAM) {
    ...
    if (ghost_newton == 1) {
      if (me == 0)
        printf("# EAM currently requires '--ghost_newton 0'; Changing setting "
               "now.\n");
  ...
  integrate.ckpt_interval = ckpt_interval; // DEPRECATED
  integrate.ckpt_dir = ckpt_dir;

  // Pass sustained I/O parameters to Integrate (Gia)
  integrate.ckpt_io_duration_sec = ckpt_io_duration_sec;
  integrate.ckpt_chunk_bytes = (size_t)ckpt_chunk_mb * 1024 * 1024;
  integrate.ckpt_sleep_us = ckpt_sleep_ms * 1000;
  integrate.ckpt_fsync_chunks = ckpt_fsync_chunks;

  // Pass communication phase parameters to Integrate (Johnnie)
  integrate.comm_phase_enabled = comm_phase_enabled;
  integrate.comm_standin_bytes = (size_t)comm_standin_mb * 1024 * 1024;
  integrate.comm_chunk_kb = comm_chunk_kb;
  ...
  if (in.datafile) {
    read_lammps_data(atom, comm, neighbor, integrate, thermo, in.datafile,
                     in.units);
  ...
  if (me == 0) {
    ...
    // Communication phase info (Johnnie)
    fprintf(stdout, "# Communication Phase Settings:\n");
    fprintf(stdout, "\t# Comm phase enabled: %i\n", comm_phase_enabled);
    fprintf(stdout, "\t# Comm stand-in size: %i MB\n", comm_standin_mb);
    fprintf(stdout, "\t# Comm chunk size: %i KB\n", comm_chunk_kb);
    fprintf(stdout, "\n");
  }
...
#pragma omp parallel
  ...
  if (me == 0) {
    ...
    printf("# MPI_proc OMP_threads nsteps natoms t_total t_force t_neigh "
           "t_comm t_other performance perf/thread grep_string t_extra\n");
