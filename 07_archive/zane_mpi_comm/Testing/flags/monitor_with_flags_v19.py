...
class PhaseAnalyzer:
    def __init__(self, expected_ranks, use_hints=False):
        self.expected_ranks = expected_ranks
        self.use_hints = use_hints

        if self.use_hints:
            print("Running in hint mode")
        ...
        # --- OPTIMIZED PARAMETERS (Generated by Scientific Tuner) ---
        self.weights = {
            'W_COMP_PWR': 4.6077,
            'W_COMP_IPC': 4.8152,
            'W_COMP_UTIL': 3.8543,
            'W_COMM_NET': 0.6481,   # Note: Low weight means Net is noisy
            'W_COMM_LOW_U': 3.3410,
            'W_COMM_VAR': 1.1954,
            'W_MEM_MISS': 2.9495,
            'W_MEM_DRAM': 3.8619
    ...
    def _normalize_metrics(self, m:SystemMetrics):
        ...
        total_cpu_cores = self.expected_ranks
    ...
    def detect_statistical_outliers(self, m: SystemMetrics):
        # We keep the outlier logic but rely mostly on the weights now
    ...
    def _stabilize_phase(self, phase):
        ...
        if self.use_hints: return phase
    ...
    def detect_phase(self, m: SystemMetrics) -> tuple[str, dict]:
        ...
        if self.use_hints:
            try:
                with open(HINT_FILE, "r") as f:
                    hint = f.read().strip()
                    if hint:
                        return hint, {"reasons": "Application Hint", "scores": "Oracle"}
            except FileNotFoundError:
                pass # Fallback if file isn't ready
            return "UNKNOWN", {"reasons": "No Hint", "scores": "Oracle"}
        
        # --------------------------------------------------------------------
        # SYSTEM READ MODE (Heuristic)
        # --------------------------------------------------------------------

        # 1. DETECT STORAGE (I/O)
        ...
        # Stabilization logic can be added here if needed
        final_phase = phase
...
class IntelligentMonitorv17:
    def __init__(self, args):
        ...
        # Pass the 'hints' argument from the command line into the Analyzer
        self.analyzer = PhaseAnalyzer(expected_ranks=args.ranks, use_hints=args.hints)
    ...
    def run(self):
        ...
        if self.analyzer.use_hints:
            os.environ["MINIMD_HINT_MODE"] = "1"
            print("[INFO] Exported MINIMD_HINT_MODE=1")
        else:
            if "MINIMD_HINT_MODE" in os.environ:
                del os.environ["MINIMD_HINT_MODE"]
...
def main():
    ...
    parser.add_argument('--hints', action='store_true', help='Enable Direct App Hints (Oracle Mode)')
