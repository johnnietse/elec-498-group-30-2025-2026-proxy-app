                    ...
                    Comm &comm, Thermo &thermo, Timer &timer)
{
  ...
  #pragma omp parallel private(i,n)
  {
    ...
    for(n = 0; n < ntimes; n++) {
      ...
      // -----------------------------------------
      //CAPSTONE CHECKPOINT LOGIC
      // -----------------------------------------
      if ((n + 1) % 200 == 0) {
          
          // Sync all OpenMP threads within this rank
          #pragma omp barrier

          // Only the Master Thread of each Rank talks to MPI
          #pragma omp master 
          {
              // BARRIER START: Everyone waits here
              // cause the threads to go idle
              MPI_Barrier(MPI_COMM_WORLD);

              // Rank 0 does the work aka writing to io
              if (comm.me == 0) {
                printf(">>> VERIFICATION: Custom checkpoint logic triggered at step %d\n", n + 1);
                  std::ofstream outfile("capstone_checkpoint.txt", std::ios_base::app);
                  
                  if (outfile.is_open()) {
                      outfile << "Checkpoint Timestamp: " << (n + 1) << "\n";
                      outfile.close();
                  } else {
                      printf("Error: Rank 0 could not open checkpoint file.\n");
                  }
              }

              //BARRIER END: No one leaves until Rank 0 is done writing.
              MPI_Barrier(MPI_COMM_WORLD);
          }

          //Sync threads again so they wake up together
          #pragma omp barrier
      }
      // -----------------------------------------
      // END: CAPSTONE CHECKPOINT LOGIC
      // -----------------------------------------
