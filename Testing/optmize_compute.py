...
CPU_SETSPEED_FILES = glob.glob("/sys/devices/system/cpu/cpu*/cpufreq/scaling_setspeed")
CPU_GOVERNOR_FILES = glob.glob("/sys/devices/system/cpu/cpu*/cpufreq/scaling_governor")
# ==========================
...
def set_system_frequency(freq_str):
    # write all the frequency to all avaible cpu cores on the node
    for gov_file, speed_file in zip(CPU_GOVERNOR_FILES, CPU_SETSPEED_FILES):
        try:
            # open in userspace
            with open(gov_file, "w") as f: f.write("userspace")

            # set the speed
            with open(speed_file, "w") as f: f.write(freq_str)
...
# Intialy run with low freq mode
...
try:
    while True:
        ...
        if current_phase != last_phase:
            ...
            if streak_counter >= STREAK_NEEDED:
                ...
                #apply DVFS from blueprint
                if current_phase in ["PARALLEL", "SERIAL"]:
                    set_system_frequency(FREQ_MAX)
                    action = "UP -> 2.0 GHz"
                else:
                    set_system_frequency(FREQ_MIN)
                    action = "DOWN -> 1.2 GHz"
                ...
                print(f"[phase @ {t:6.2f}s] {current_phase:15s} | {action} | util={total_util:3.1f} | apps={app_threads} | helpers={active_helpers} | IPC={ipc:.2f} | watts={watts:.1f}")
    ...
    print("\n[cleanup] Resetting CPUs to Performance mode...")
    # Attempt to reset to MAX or back to 'ondemand/performance' governor
    for gov_file in CPU_GOVERNOR_FILES:
        try:
            with open(gov_file, "w") as f: f.write("performance")
        except: pass
