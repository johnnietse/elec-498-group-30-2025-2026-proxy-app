...
int main(int argc, char** argv)
{
  ...
  int ckpt_interval = 0;           // 0 disables
  const char* ckpt_dir = "chk";    // default output directory

  // I/O scaling parameters (for 30-second sustained I/O)
  double ckpt_ioscale_sec = 30.0;      // seconds to keep I/O active
  int ckpt_ioscale_chunk_mb = 1;       // chunk size (MiB) per write
  int ckpt_ioscale_sleep_ms = 100;     // sleep between chunk writes
  ...
  for(int i = 0; i < argc; i++) {
    ...
    // I/O-scaling CLI flags
    if((strcmp(argv[i], "--ckpt_ioscale_sec") == 0)) {
      ckpt_ioscale_sec = atof(argv[++i]);
      continue;
    }
    if((strcmp(argv[i], "--ckpt_ioscale_chunk_mb") == 0)) {
      ckpt_ioscale_chunk_mb = atoi(argv[++i]);
      continue;
    }
    if((strcmp(argv[i], "--ckpt_ioscale_sleep_ms") == 0)) {
      ckpt_ioscale_sleep_ms = atoi(argv[++i]);
      continue;
    }
    ...
    if((strcmp(argv[i], "-h") == 0) || (strcmp(argv[i], "--help") == 0)) {
      ...
      printf("\n  Checkpointing:\n");
      printf("\t--ckpt <K>:                   checkpoint every K steps (0 disables)\n");
      printf("\t--ckpt_dir <DIR>:             checkpoint directory (default: chk)\n");
      printf("\t--ckpt_ioscale <0|1>:         enable I/O scaling sidecar writes (default 0)\n");
      printf("\t--ckpt_ioscale_sec <S>:       I/O phase duration in seconds (default 30)\n");
      printf("\t--ckpt_ioscale_chunk_mb <MB>: write chunk size in MiB (default 1)\n");
      printf("\t--ckpt_ioscale_sleep_ms <MS>: sleep between chunks in ms (default 100)\n");
      printf("\t--ckpt_ioscale_fsync <0|1>:   fsync each chunk (default 0)\n");
  ...
  integrate.ckpt_interval = ckpt_interval;
  integrate.ckpt_dir = ckpt_dir;

  // Pass I/O scaling parameters to Integrate
  integrate.ckpt_ioscale_sec = ckpt_ioscale_sec;
  integrate.ckpt_ioscale_chunk_bytes = (size_t) ckpt_ioscale_chunk_mb * 1024 * 1024;
  integrate.ckpt_ioscale_sleep_us = ckpt_ioscale_sleep_ms * 1000;
