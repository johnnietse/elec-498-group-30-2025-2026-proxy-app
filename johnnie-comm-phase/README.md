...
while all other ranks wait at `MPI_Barrier`. Any graphs in this directory are not the latest graphs that are shown in the final report but are there to see what kinds of graphs shall we draft for the final report for comparison, so it is just for initial testing and plan draw out for what kind of graphs shall be put into the final report. The graphs in the final report are the finalized graphs, figures, tables, etc.
