#!/bin/bash
#SBATCH --job-name=minimd_test_a
#SBATCH --output=test_a_%j.out
#SBATCH --nodes=1
#SBATCH --ntasks=30
...
# ============================================================
# TEST A — BASELINE (no communication phase, performance governor)
#
# Runs miniMD with --comm_phase 0 (comm phase disabled).
# All cores stay at default frequency under "performance" governor.
# This is the control run to compare against Tests B and C.
#
...
# ---- Output ----
CSV="results_test_a_${SLURM_JOB_ID}.csv"
SUMMARY_LOG="summary_test_a_${SLURM_JOB_ID}.txt"
...
cleanup() {
    ...
    # Kill any lingering background samplers
    rm -f .running_* .energy_tmp_* 2>/dev/null || true
    echo "[CLEANUP] Done."
}
...
# ---- Banner ----
echo "============================================================"
echo "TEST A — BASELINE (comm_phase OFF)"
...
# ---- CPU Info (from Gia's checkpoint_freq_full.sh) ----
echo ""
...
# Background energy sampler (from val_testing/automate_baseline.sh)
# Polls RAPL every 10s, accumulates across wraparounds
sample_energy() {
    local RUN_ID=$1
...
# ---- Idle Baseline Measurement (from val_testing/automate_scaling.sh) ----
...
# ---- CSV Header ----
echo "run,nprocs,energy_j,dynamic_energy_j,avg_power_w,idle_power_w,t_total,t_force,t_neigh,t_comm,t_other,performance,io_dur_s,io_bw_mbps" > "$CSV"
...
for N in $MPI_COUNTS; do
    ...
    # Ensure performance governor on all cores we'll use
    reset_governors 29

    # Verify freq after governor reset
    ...
    for run in $(seq 1 $NUM_RUNS); do
		...
		# Temporary log file
        TMPLOG=$(mktemp /tmp/minimd_a_${N}_${run}_XXXXXX.log)

		# Read energy BEFORE
		BEFORE_UJ=$(read_energy_uj)
		
        # Start background energy sampler
        RUN_ID="a_${N}_${run}"
        ...
        # Run miniMD — comm phase DISABLED
        numactl --localalloc \
            mpirun -np $N --map-by core --bind-to core \
            "$BINARY" -i "$INPUT" --comm_phase 0 \
            2>&1 | tee "$TMPLOG"
        ...
        # Calculate energy from sampler (handles wraparound automatically)
        ...
        # Write CSV row
        echo "$run,$N,$ENERGY_J,$DYNAMIC_J,$AVG_POWER,$IDLE_POWER_W,$T_TOTAL,$T_FORCE,$T_NEIGH,$T_COMM,$T_OTHER,$PERFORMANCE,$IO_DUR,$IO_BW" >> "$CSV"


        echo "  Energy: ${ENERGY_J} J (dynamic: ${DYNAMIC_J} J) | Power: ${AVG_POWER} W | Time: ${T_TOTAL} s | Perf: ${PERFORMANCE}"
...
{
    echo "============================================================"
    echo "TEST A — SUMMARY (Baseline, comm_phase OFF)"
...
# ---- Create Results Tarball (from Gia's checkpoint_freq_full.sh) ----
TARBALL="test_a_results_${SLURM_JOB_ID}.tar.gz"
echo "Creating results tarball: $TARBALL"
tar -czf "$TARBALL" "$CSV" "$SUMMARY_LOG" "test_a_${SLURM_JOB_ID}.out" 2>/dev/null || true
if [ -f "$TARBALL" ]; then
...
# Restore governors (also in trap, but be explicit)
reset_governors 29
echo ""
